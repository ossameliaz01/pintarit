    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6" style={{ background: '#F8F8FC' }}>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="sm:col-span-2 lg:col-span-1 bg-white rounded-2xl p-5 flex flex-col items-center"
          ...
          <p className="text-sm font-semibold mb-2" style={{ color: '#969696' }}>Total XP</p>
        ...
        {[
          ...
          <div key={i} className="bg-white rounded-2xl p-5" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
            ...
            <p className="text-3xl font-bold" style={{ color: '#372466' }}>{s.value}</p>
            <p className="text-sm font-semibold" style={{ color: '#333333' }}>{s.label}</p>
            <p className="text-sm mt-1" style={{ color: '#969696' }}>{s.sub}</p>
          </div>
        ))}
      </div>
          ...
          {badges.map((badge, i) => {
            ...
            return (
              <div key={i} className={`rounded-2xl p-4 text-center card-hover transition-all duration-200`}
                ...
                <div className={`text-5xl mb-2 transition-all duration-300 ${badge.unlocked ? '' : 'grayscale opacity-40'}`}
                  style={{ filter: badge.unlocked ? 'none' : 'grayscale(1)' }}>
                  {badge.icon}
                </div>
                <p className="text-sm font-bold mb-1" style={{ color: badge.unlocked ? '#372466' : '#969696' }}>{badge.name}</p>
                <span className="text-sm px-2 py-0.5 rounded-full font-medium"
                  style={{ background: rc.bg, color: rc.color }}>
                  {badge.rarity}
                </span>
                <p className="text-sm mt-2 leading-tight" style={{ color: '#969696' }}>{badge.desc}</p>
                <p className="text-sm font-bold mt-2" style={{ color: badge.unlocked ? badge.color : '#969696' }}>
                  ⚡ {badge.xpReq} XP
                </p>
                ...
                {!badge.unlocked && badge.progress !== undefined && badge.progress > 0 && (
                  <div className="mt-2">
                    ...
                    <p className="text-sm mt-0.5" style={{ color: '#969696' }}>{badge.progress}%</p>
                  </div>
                )}

                {!badge.unlocked && (!badge.progress || badge.progress === 0) && (
                  <p className="text-sm mt-2">🔒 Terkunci</p>
                )}

                {badge.unlocked && (
                  <div className="mt-2 text-sm font-semibold" style={{ color: '#4CAF50' }}>✅ Diraih!</div>
                )}
              </div>
            )
          })}
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        {/* Recent Rewards */}
        <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
          <h3 className="font-bold text-base mb-4" style={{ color: '#372466' }}>🎁 Reward Terbaru</h3>
            ...
            {recentRewards.map((r, i) => (
              <div key={i} className="flex items-center gap-3 p-3 rounded-xl"
                ...
                <div className="flex-1">
                  <p className="text-sm font-semibold" style={{ color: '#333333' }}>{r.name}</p>
                  <p className="text-sm" style={{ color: '#969696' }}>{r.time}</p>
                </div>
        ...
        <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-base" style={{ color: '#372466' }}>📅 Misi Mingguan</h3>
            ...
            {weeklyMissions.map((m, i) => (
              <div key={i} className="p-3 rounded-xl" style={{ background: '#F8F8FC', border: '1px solid #F0EFF8' }}>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-sm font-bold" style={{ color: m.done ? '#4CAF50' : '#9568FF' }}>+{m.xp} XP</span>
                </div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm" style={{ color: '#969696' }}>{m.progress}/{m.total}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 p-3 rounded-xl"
            ...
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-bold" style={{ color: '#372466' }}>Total Misi Mingguan</p>
                <p className="text-sm" style={{ color: '#969696' }}>2 dari 4 selesai</p>
              </div>
              <div className="text-right">
                <p className="text-base font-bold" style={{ color: '#9568FF' }}>550 XP</p>
                <p className="text-sm" style={{ color: '#969696' }}>total reward</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="font-bold text-base" style={{ color: '#372466' }}>🔥 Daily Streak</h3>
            <p className="text-sm" style={{ color: '#969696' }}>Konsistensi adalah kunci sukses belajar</p>
          </div>
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl"
            style={{ background: 'rgba(255,112,67,0.1)', border: '1px solid rgba(255,112,67,0.2)' }}>
            <Flame size={16} style={{ color: '#FF7043' }} />
            <span className="font-bold text-base" style={{ color: '#FF7043' }}>7 hari</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-7 gap-2">
          {['Sen', 'Sel', 'Rab', 'Kam', "Jum'", 'Sab', 'Min'].map((day, i) => (
            <div key={i} className="text-center">
              <p className="text-sm mb-1" style={{ color: '#969696' }}>{day}</p>
        ...
        <div className="flex gap-4 mt-4 justify-center">
          {[
            ...
            <div key={i} className="text-center px-4 py-2 rounded-xl" style={{ background: '#F8F8FC' }}>
              <p className="text-base font-bold" style={{ color: '#372466' }}>{s.value}</p>
              <p className="text-sm" style={{ color: '#969696' }}>{s.label}</p>
            </div>
          ))}
        </div>
      </div>
