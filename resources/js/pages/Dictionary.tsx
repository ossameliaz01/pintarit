    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-5" style={{ background: '#F8F8FC' }}>
      ...
      <div className="rounded-2xl p-5 relative overflow-hidden"
        ...
        <div className="relative">
          <h2 className="text-lg font-bold text-white mb-1">📚 Kamus IT Interaktif</h2>
          ...
          <div className="relative">
            <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: '#9568FF' }} />
            <input
              value={search}
      ...
      <div className="grid lg:grid-cols-3 gap-5" ref={containerRef}>
        ...
        <div className="lg:col-span-2">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-sm" style={{ color: '#372466' }}>{t.term}</span>
                    ...
                    <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(149,104,255,0.1)', color: '#9568FF' }}>
                      {t.category}
                    </span>
                    <p className="text-xs mt-2 leading-relaxed" style={{ color: '#969696' }}>{t.short}</p>
                  </div>
                </div>

                {selected?.term === t.term && (
                  <div className="mt-4 pt-4" style={{ borderTop: '1px solid #F0EFF8' }}>
                    <p className="text-xs leading-relaxed mb-3" style={{ color: '#333333' }}>{t.definition}</p>
                    <div className="p-3 rounded-xl mb-3" style={{ background: 'rgba(149,104,255,0.06)', border: '1px solid rgba(149,104,255,0.15)' }}>
                      <p className="text-xs" style={{ color: '#333333' }}>{t.analogy}</p>
                    </div>
                    <div className="p-3 rounded-xl" style={{ background: '#F8F8FC', border: '1px solid #E8E6F0' }}>
                      <p className="text-xs font-semibold mb-1" style={{ color: '#372466' }}>Contoh:</p>
                      <p className="text-xs font-mono" style={{ color: '#9568FF' }}>{t.example}</p>
                    </div>
                  </div>
                )}
              </div>
          ...
          {filtered.length === 0 && (
            <div className="bg-white rounded-2xl p-12 text-center" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
              ...
              <p className="font-semibold text-sm" style={{ color: '#372466' }}>Tidak ada hasil ditemukan</p>
              <p className="text-xs mt-1" style={{ color: '#969696' }}>Coba kata kunci yang berbeda</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl p-5" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
            <h4 className="font-bold text-sm mb-3 flex items-center gap-2" style={{ color: '#372466' }}>
              <Star size={14} style={{ color: '#FFC107', fill: '#FFC107' }} /> Favorit Saya
            </h4>
            {favorites.length === 0 ? (
              <p className="text-xs" style={{ color: '#969696' }}>Belum ada istilah favorit</p>
            ) : (
              <div className="space-y-2">
                {terms.filter(t => favorites.includes(t.term)).map((t, i) => (
                  <div key={i} className="flex items-center gap-2 p-2 rounded-lg cursor-pointer"
                    ...
                    <span>{t.icon}</span>
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-semibold truncate" style={{ color: '#333333' }}>{t.term}</p>
                    </div>
                    <ChevronRight size={12} style={{ color: '#969696' }} />
                  </div>
                ))}
              </div>
            )}
          ...
          {/* Recent terms */}
          <div className="bg-white rounded-2xl p-5" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
            <h4 className="font-bold text-sm mb-3" style={{ color: '#372466' }}>🕐 Terakhir Dilihat</h4>
            <div className="space-y-2">
              {['IP Address', 'DNS', 'VLAN', 'Firewall'].map((term, i) => (
                <div key={i} className="flex items-center gap-2 p-2 rounded-lg cursor-pointer"
                  style={{ background: '#F8F8FC' }}
                  onClick={() => setSelected(terms.find(t => t.term === term) || null)}>
                  <BookOpen size={12} style={{ color: '#9568FF' }} />
                  <span className="text-xs" style={{ color: '#333333' }}>{term}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-2xl p-5"
            ...
            <h4 className="font-bold text-sm mb-3 text-white">📊 Statistik Kamus</h4>
            {[
        ...
        <div className="fixed z-50 pointer-events-none tooltip-enter"
          ...
          <div className="bg-white rounded-2xl p-4 shadow-2xl" style={{ boxShadow: '0 16px 48px rgba(55,36,102,0.25)', border: '1px solid rgba(149,104,255,0.2)' }}>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xl">{tooltip.term.icon}</span>
              <span className="font-bold text-sm" style={{ color: '#372466' }}>{tooltip.term.term}</span>
            </div>
            <p className="text-xs leading-relaxed" style={{ color: '#969696' }}>{tooltip.term.definition}</p>
          </div>
        </div>
      )}
    </div>
