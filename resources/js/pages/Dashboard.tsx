    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6" style={{ background: '#F8F8FC' }}>
      ...
      <div className="rounded-2xl p-6 relative overflow-hidden"
        ...
        <div className="relative flex flex-wrap gap-6 items-start justify-between">
          <div>
            <p className="text-sm mb-1" style={{ color: 'rgba(255,255,255,0.7)' }}>Selamat datang kembali 👋</p>
            <h2 className="text-xl font-bold text-white mb-1">Rizki Pratama</h2>
            <p className="text-xs" style={{ color: 'rgba(255,255,255,0.6)' }}>SMK TKJ Bandung · Network Warrior</p>
          </div>
        ...
        <div className="relative mt-5">
          ...
          <p className="text-xs mt-1" style={{ color: 'rgba(255,255,255,0.5)' }}>160 XP lagi untuk naik level!</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {quickActions.map((qa, i) => (
          <button key={i} onClick={() => onNavigate(qa.page)}
            ...
            <span className="text-xs font-semibold" style={{ color: '#372466' }}>{qa.label}</span>
          </button>
        ))}
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-6" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-bold text-sm" style={{ color: '#372466' }}>XP Mingguan</h3>
              <p className="text-xs" style={{ color: '#969696' }}>Total: 2,470 XP pekan ini</p>
            </div>
        ...
        <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-sm" style={{ color: '#372466' }}>Leaderboard</h3>
            <span className="text-xs" style={{ color: '#9568FF' }}>Minggu ini</span>
          </div>
          <div className="space-y-3">
            {leaderboard.map((user, i) => (
              <div key={i} className={`flex items-center gap-3 p-2 rounded-xl ${user.isMe ? 'border' : ''}`}
                ...
                <span className="text-base w-5 text-center">{user.badge}</span>
                <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white"
                  style={{ background: 'linear-gradient(135deg, #372466, #9568FF)', flexShrink: 0 }}>
                  {user.name[0]}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold truncate" style={{ color: user.isMe ? '#372466' : '#333333' }}>
                    {user.name} {user.isMe && <span style={{ color: '#9568FF' }}>•Kamu</span>}
                  </p>
                  <p className="text-xs truncate" style={{ color: '#969696' }}>Lv.{user.level}</p>
                </div>
                <span className="text-xs font-bold" style={{ color: '#9568FF' }}>{user.xp.toLocaleString()}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-sm" style={{ color: '#372466' }}>Modul Terkini</h3>
            <button onClick={() => onNavigate('roadmap')} className="flex items-center gap-1 text-xs font-medium" style={{ color: '#9568FF' }}>
            ...
            {recentModules.map((m, i) => (
              <div key={i} className={`flex items-center gap-3 p-3 rounded-xl transition-all duration-200 cursor-pointer ${!m.locked ? 'hover:bg-purple-50' : ''}`}
                ...
                <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg flex-shrink-0"
                  ...
                  {m.locked ? <Lock size={16} style={{ color: '#969696' }} /> : m.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold truncate" style={{ color: m.locked ? '#969696' : '#333333' }}>{m.name}</p>
                  <div className="flex items-center gap-2 mt-1">
                    {m.progress > 0 && !m.locked && (
                      <>
                        ...
                        <span className="text-xs" style={{ color: '#969696' }}>{m.progress}%</span>
                      </>
                    )}
                  </div>
                  <div className="flex items-center gap-3 mt-1">
                    <span className="text-xs" style={{ color: '#9568FF' }}>⚡ {m.xp} XP</span>
                    <span className="text-xs" style={{ color: '#969696' }}>⏱ {m.time}</span>
                  </div>
                </div>
                {!m.locked && (
                  <button className="w-7 h-7 rounded-lg flex items-center justify-center"
                    ...
                    <Play size={12} style={{ color: '#9568FF' }} />
                  </button>
                )}
              </div>
            ))}
        ...
        <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-bold text-sm" style={{ color: '#372466' }}>Misi Harian</h3>
              <p className="text-xs" style={{ color: '#969696' }}>2 / 4 selesai</p>
            </div>
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold"
              style={{ background: 'rgba(255,193,7,0.1)', color: '#FFC107' }}>
              <Flame size={12} /> 7 hari
            </div>
          </div>
          ...
          <div className="mb-4">
            ...
            <p className="text-xs mt-1" style={{ color: '#969696' }}>290 / 580 XP bonus hari ini</p>
          </div>
            ...
            {missions.map((m, i) => (
              <div key={i} className="flex items-center gap-3 p-3 rounded-xl mission-stripe"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs font-semibold mt-0.5" style={{ color: m.done ? '#4CAF50' : '#9568FF' }}>{m.reward}</p>
                </div>
                <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0"
                  style={{ background: m.done ? '#4CAF50' : '#F0EFF8' }}>
                  <span className="text-xs" style={{ color: m.done ? 'white' : 'transparent' }}>✓</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-5 p-3 rounded-xl" style={{ background: 'rgba(149,104,255,0.06)', border: '1px dashed rgba(149,104,255,0.3)' }}>
            <p className="text-xs font-semibold mb-1" style={{ color: '#9568FF' }}>🎯 Challenge Minggu Ini</p>
            <p className="text-xs" style={{ color: '#969696' }}>Selesaikan Modul Subnetting dalam 5 hari</p>
            <div className="flex items-center gap-2 mt-2">
              ...
              <span className="text-xs font-semibold" style={{ color: '#9568FF' }}>35%</span>
            </div>
            <p className="text-xs mt-1" style={{ color: '#969696' }}>Reward: 500 XP + Badge "Subnet Master"</p>
          </div>
        </div>
      </div>
    </div>
