  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6" style={{ background: '#F8F8FC' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-white rounded-2xl p-5 card-hover" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
            ...
            <p className="text-xl font-bold" style={{ color: '#372466' }}>{stat.value}</p>
            <p className="text-xs font-semibold" style={{ color: '#333333' }}>{stat.label}</p>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-6" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-bold text-sm" style={{ color: '#372466' }}>XP Kelas per Minggu</h3>
              <p className="text-xs" style={{ color: '#969696' }}>Total aktivitas belajar seluruh siswa</p>
            </div>
        ...
        <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
          <h3 className="font-bold text-sm mb-4" style={{ color: '#372466' }}>Distribusi Badge</h3>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        {/* Topic Completion */}
        <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
          <h3 className="font-bold text-sm mb-4" style={{ color: '#372466' }}>📊 Penyelesaian per Topik</h3>
        ...
        <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-sm" style={{ color: '#372466' }}>⚠️ Topik Lemah Siswa</h3>
            ...
            {weakTopics.map((t, i) => (
              <div key={i} className="flex items-center gap-4 p-3 rounded-xl"
                ...
                <div className="flex-1">
                  <p className="text-xs font-semibold" style={{ color: '#333333' }}>{t.name}</p>
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-xs font-bold" style={{ color: '#EF4444' }}>{t.score}%</span>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-xs font-bold" style={{ color: '#333333' }}>{t.students}</p>
                  <p className="text-xs" style={{ color: '#969696' }}>siswa</p>
                </div>
              </div>
            ))}
          </div>
          <button className="w-full mt-4 py-2.5 rounded-xl text-xs font-semibold text-white"
      ...
      <div className="bg-white rounded-2xl p-6" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
        <div className="flex items-center justify-between mb-5">
          <div>
            <h3 className="font-bold text-sm" style={{ color: '#372466' }}>👥 Progress Siswa</h3>
            <p className="text-xs" style={{ color: '#969696' }}>32 siswa aktif · Kelas XII TKJ-A</p>
          </div>
          <button className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold text-white"
            style={{ background: 'linear-gradient(135deg, #372466, #9568FF)' }}>
            <Download size={12} /> Download Laporan
          </button>
        </div>
            ...
            <thead>
              <tr style={{ borderBottom: '2px solid #F0EFF8' }}>
                {['Siswa', 'Level', 'XP Total', 'Progress', 'Streak', 'Modul', 'Badge'].map(h => (
                  <th key={h} className="pb-3 text-left text-xs font-semibold pr-4" style={{ color: '#969696' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={i} style={{ borderBottom: '1px solid #F8F8FC' }}
                  ...
                  <td className="py-3 pr-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium" style={{ color: '#333333' }}>{s.name}</span>
                    </div>
                  </td>
                  <td className="py-3 pr-4">
                  ...
                  <td className="py-3 pr-4 text-xs font-semibold" style={{ color: '#333333' }}>{s.xp.toLocaleString()}</td>
                  <td className="py-3 pr-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs" style={{ color: '#969696' }}>{s.progress}%</span>
                    </div>
                  </td>
                  <td className="py-3 pr-4 text-xs" style={{ color: '#FF7043' }}>🔥 {s.streak}d</td>
                  <td className="py-3 pr-4 text-xs font-medium" style={{ color: '#333333' }}>{s.modules}</td>
                  <td className="py-3">
                    <span className="text-xs px-2 py-0.5 rounded-full" style={{ background: 'rgba(149,104,255,0.08)', color: '#9568FF' }}>
                      {s.badge}
                    </span>
                  </td>
                </tr>
