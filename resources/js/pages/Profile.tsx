    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-6" style={{ background: '#F8F8FC' }}>
      ...
      <div className="bg-white rounded-2xl overflow-hidden" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
        ...
        <div className="px-6 pb-6">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-4">
            <div className="w-24 h-24 rounded-2xl border-4 border-white flex items-center justify-center text-4xl font-bold text-white shrink-0 -mt-12 relative z-10"
              style={{ background: 'linear-gradient(135deg, #9568FF, #B794F6)', boxShadow: '0 8px 24px rgba(149,104,255,0.4)' }}>
              RP
            </div>
            <button className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold mt-4 sm:mt-0 hover:bg-purple-50 transition-colors duration-200"
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
            <div className="flex-1">
              <h2 className="text-3xl font-bold" style={{ color: '#372466' }}>Rizki Pratama</h2>
              <p className="text-base font-medium mt-1" style={{ color: '#9568FF' }}>Network Warrior · Level 8</p>
              
              <div className="flex flex-wrap items-center gap-4 mt-3">
                <div className="flex items-center gap-1.5 text-sm font-medium" style={{ color: '#7A7A9D' }}>
                  <School size={16} /> SMK TKJ Bandung
                </div>
                <div className="flex items-center gap-1.5 text-sm font-medium" style={{ color: '#7A7A9D' }}>
                  <Calendar size={16} /> Bergabung Jan 2024
                </div>
              </div>
              
              <p className="text-base mt-4 leading-relaxed max-w-2xl" style={{ color: '#555570' }}>
            ...
            <div className="grid grid-cols-3 gap-3 w-full md:w-auto shrink-0 mt-2 md:mt-0">
              {[
                ...
                <div key={i} className="text-center px-4 py-3 rounded-xl min-w-[90px]" style={{ background: '#F8F8FC', border: '1px solid #F0EFF8' }}>
                  ...
                  <p className="text-sm font-medium mt-1" style={{ color: '#7A7A9D' }}>{s.label}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl p-5" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
          <h3 className="font-bold text-base mb-4" style={{ color: '#372466' }}>🏆 Badge Diraih</h3>
          <div className="grid grid-cols-2 gap-3">
            {['🌱', '🌐', '🗺️', '🐧'].map((icon, i) => (
              <div key={i} className="text-center p-3 rounded-xl card-hover"
                ...
                <p className="text-sm font-semibold" style={{ color: '#372466' }}>
        ...
        {/* Learning History */}
        <div className="bg-white rounded-2xl p-5" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
          <h3 className="font-bold text-base mb-4" style={{ color: '#372466' }}>📚 Riwayat Belajar</h3>
            ...
            {history.map((h, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate" style={{ color: '#333333' }}>{h.title}</p>
                  <p className="text-sm" style={{ color: '#969696' }}>{h.date}</p>
                </div>
                <span className="text-sm font-bold" style={{ color: '#9568FF' }}>+{h.xp}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Certificates */}
        <div className="bg-white rounded-2xl p-5" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
          <h3 className="font-bold text-base mb-4" style={{ color: '#372466' }}>📜 Sertifikat</h3>
          <div className="space-y-3">
            {certs.map((c, i) => (
              <div key={i} className="p-3 rounded-xl"
                ...
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button className="flex items-center gap-1 text-sm" style={{ color: '#9568FF' }}>
                    <Download size={10} /> PDF
                  </button>
                </div>
                <p className="text-sm font-semibold" style={{ color: '#372466' }}>{c.name}</p>
                <p className="text-sm" style={{ color: '#969696' }}>{c.issuer} · {c.date}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl p-5" style={{ boxShadow: '0 4px 16px rgba(55,36,102,0.07)' }}>
        <h3 className="font-bold text-base mb-4" style={{ color: '#372466' }}>⚙️ Pengaturan Akun</h3>
          ...
          {[
            ...
            <div key={i} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: '#F8F8FC' }}>
              ...
              <div>
                ...
                <p className="text-sm" style={{ color: '#9568FF' }}>{s.value}</p>
              </div>
            </div>
          ))}
        </div>
